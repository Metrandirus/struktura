...
# Бренды трековых систем — если они есть в общей выгрузке, попадут в
# data/track-products.json. Если нет (выгрузка может не включать трек-системы
# вовсе) — файл останется пустым списком, сайт корректно покажет "наполняется".
TRACK_BRANDS = ["MayFar", "Maytoni", "Artelamp", "StLuce", "Novotech", "Arlight"]

# Ключевые слова для определения типа трековой системы по названию/серии товара.
TRACK_SYSTEM_KEYWORDS = {
    "single": ["однофазн", "однофаз"],
    "three": ["трёхфазн", "трехфазн", "3-фазн", "трехфаз"],
    "magnetic": ["магнитн"],
}
...
def guess_track_system(text: str):
    if not isinstance(text, str):
        return None
    low = text.lower()
    for sys_id, keywords in TRACK_SYSTEM_KEYWORDS.items():
...
def build_records(df: pd.DataFrame, brand_key: str, name_col, type_col, art_col, price_col, stock_col, photo_col, series_col=None, manu_col=None):
    ...
    for _, r in df.iterrows():
        ...
        if brand_key == "track":
            record["br"] = str(r[manu_col]).strip() if manu_col else None
            record["sys"] = guess_track_system(name) or guess_track_system(str(r.get(series_col, "")))
...
def main():
    ...
    # Трековые системы — лучшее, что можно сделать без подтверждённой структуры
    # выгрузки: ищем строки, где производитель — один из TRACK_BRANDS. Если в
    # этой выгрузке трек-систем нет вообще, получится пустой список, и сайт
    # покажет "каталог наполняется" — это ожидаемо, не ошибка.
    track_mask = df[manu_col].astype(str).str.strip().str.lower().isin([b.lower() for b in TRACK_BRANDS])
    track_df = df[track_mask].copy()
    track_records = build_records(
        track_df, "track", name_col, type_col, art_col, price_col, stock_col, photo_col, series_col, manu_col
    )
    ...
    if len(track_records) == 0:
        log("ℹ️  Трек-системы не найдены в этой выгрузке — это нормально, если поставщик "
            "не включает их в общий фид. Понадобится отдельный источник данных.")
